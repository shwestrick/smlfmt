...
functor OrderMapImpl (type t val compare : t * t -> order)
        : ORDER_MAP_IMPL where type dom = t =
  struct
    fun die s = raise Fail ("OrderMap." ^ s)
    ...
    fun impossible s = raise (Impossible ("OrderMap" ^ s))
    ...
    *)
    ...
    fun merge (f:'b * 'b -> 'b) (t1:'b map) (t2:'b map) : 'b map =
      let
        (* merge lists *)
	fun merg ([], ys) = ys
	  | merg (xs, []) = xs
	  | merg (l as((d1,r1)::xs), l' as ((d2,r2)::ys)) =
	    if lt(d1,d2) then (d1,r1):: merg(xs, l')
	    else if lt(d2,d1) then (d2,r2)::merg(l, ys)
		 else (* d1=d2 *)
		   (d1, f(r1,r2))::merg(xs,ys)
      in
         fromSortedList(merg(list t1, list t2))
      end
        ...
        foldl(fn (d, acc) =>
		 case lookup m d
		   of SOME res => add(d,res,acc)
		    | NONE => raise Restrict d) empty dom
