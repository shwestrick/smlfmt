    ...
    ^ ")" ^ "\n") *)
      ...
      and loop_inString s (args as {stringStart}) =
        if is backslash at s then
          ...
          success (mk Token.StringConstant (stringStart, s+1))
      ...
      and loop_inStringEscapeSequence s (args as {stringStart}) =
        if check LexUtils.isValidSingleEscapeChar at s then
          (** Includes e.g. \t, \n, \", \\, etc. *)
          loop_inString (s+1) args
        else if check LexUtils.isValidFormatEscapeChar at s then
          loop_inStringFormatEscapeSequence (s+1)
            { stringStart = stringStart
            , escapeStart = s-1
            }
        else if is #"^" at s then
          loop_inStringControlEscapeSequence (s+1) args
        else if is #"u" at s then
          loop_inStringFourDigitEscapeSequence (s+1) args
        else if check LexUtils.isDecDigit at s then
          (** Note the `s` instead of `s+1`... intentional. *)
          loop_inStringThreeDigitEscapeSequence s args
      ...
      (** In a string, expecting to see \uxxxx
        ...
        *)
      and loop_inStringFourDigitEscapeSequence s args =
        if
      ...
      (** Inside a string, expecting to see \^c
        ...
        *)
      and loop_inStringControlEscapeSequence s (args as {stringStart}) =
      ...
      (** Inside a string, expecting to be inside a \f...f\
        ...
        *)
      and loop_inStringFormatEscapeSequence s (args as {stringStart, escapeStart}) =
        if is backslash at s then
          loop_inString (s+1) {stringStart=stringStart}
        else if check LexUtils.isValidFormatEscapeChar at s then
          loop_inStringFormatEscapeSequence (s+1) args
        else if isEndOfFileAt s then
          error
